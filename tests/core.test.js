...
assert.equal(core.sanitizePublisher("Bridal Guide NYC!"), "bridal_guide_nyc");
const publisherLink = new URL(core.publisherLinkUrl({ publisher: "Bridal Guide NYC!", shape: "round", carat: 2.25 }));
assert.equal(publisherLink.searchParams.get("via"), "publisher");
assert.equal(publisherLink.searchParams.get("partner"), "bridal_guide_nyc");
assert.equal(publisherLink.searchParams.get("shape"), "round");
assert.equal(publisherLink.searchParams.get("carat"), "2.25");
const cardLink = new URL(core.shareCardUrl("oval", 1.85));
assert.equal(cardLink.searchParams.get("via"), "share_card");
assert.equal(cardLink.searchParams.get("shape"), "oval");
assert.equal(cardLink.searchParams.get("carat"), "1.85");
